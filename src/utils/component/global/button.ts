...
export class Button {
  ...
  private init(): boolean {
    ...
    // Cherche .button_main_hover dans le wrapper
    const hoverElement = hoverWrapperElement.querySelector('.button_main_hover') as HTMLElement;
    ...
    // Cherche l'élément animal optionnel
    const animalElement = this.block.querySelector('.button_main_animal') as HTMLElement;

    this.DOM = {
      button: this.block,
      hoverWrapper: hoverWrapperElement,
      hover: hoverElement,
      animal: animalElement || undefined,
    };
  ...
  private initEvents(): void {
    ...
    this.handleMouseLeave = (e: MouseEvent) => {
      ...
      // Animation de sortie de l'élément animal - retour à la position initiale
      if (this.DOM.animal) {
        gsap.killTweensOf(this.DOM.animal);
        gsap.to(this.DOM.animal, {
          opacity: 0,
          scale: 0,
          yPercent: 0,
          duration: 0.3,
 ...
 * ```typescript
 * // Initialisation
 * const instances = initButtonHover();
 *
 * // Nettoyage plus tard
 * destroyAllButtons();
 * ```
 */
export function initButtonHover(): Button[] {
  const buttonElements = document.querySelectorAll<HTMLElement>('[data-wf--button-main--variant]');
