...
var parallaxInvertTrigger = null;
...
var initApprocheParallaxInvert = () => {
  ...
  const heroSection = document.querySelector(".section_hero");
  if (!heroSection) return;
  gsapWithCSS.set(parallaxInvertElements, {
    willChange: "transform",
    force3D: true
  });
  const tl = gsapWithCSS.timeline();
  ...
  parallaxInvertTrigger = ScrollTrigger2.create({
    trigger: heroSection,
    start: "top top",
    end: "bottom top",
    scrub: 1,
...
function initCmsCardsSlider() {
  ...
  swipers.forEach((swiperEl) => {
    const parent = swiperEl.parentElement;
    const paginationEl = parent?.querySelector(".swiper-pagination") || swiperEl.querySelector(".swiper-pagination");
    new swiperSetup_default(swiperEl, {
      direction: "horizontal",
      loop: true,
      //   centeredSlides: true,
      spaceBetween: 2 * 16,
...
function initFsAttributesScripts() {
  if (typeof window !== "undefined" && window.FinsweetAttributes) {
    return;
  }
