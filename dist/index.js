...
var initPreloader = () => {
  ...
  if (!shouldShowPreloader()) {
    const path = window.location.pathname;
    const isHomePage = path === "/" || path === "/index.html" || path === "";
    const minHoldMs = isHomePage ? 0 : 1e3;
    ...
    const lineElement2 = document.querySelector('[preloader="loading-line"]');
    if (lineElement2) {
      gsapWithCSS.set(lineElement2, { width: "0%" });
    }
    initPreloaderVideo();
    const counter = { value: 0 };
    gsapWithCSS.to(counter, {
      value: 100,
      duration: minHoldMs / 1e3,
      ease: "power2.out",
      onUpdate: () => updateLoadingDisplay(counter.value)
    });
    gsapWithCSS.delayedCall(minHoldMs / 1e3, () => {
      window.dispatchEvent(new CustomEvent("preloaderComplete"));
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
